...
def simple_gradient(x, y, theta):
    ...
    gradient_intercept = np.sum(full_array.dot(theta) - y) / len(y)
    gradient_slope = np.sum((full_array.dot(theta) - y)*x) / len(y)
    return np.array([gradient_intercept, gradient_slope])
